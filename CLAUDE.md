...
12. **쿼리스트링 직렬화에 `encodeURIComponent(value)` 직접 금지 (BUG-009)**: 인자에 `String()`이 강제로 걸려 객체·배열이 조용히 `[object Object]`가 된다. 값이 원시값이 아닐 수 있는 자리는 재귀 직렬화(`buildQueryString` — 중첩은 브래킷 표기 `page[selectPage]=1`, qs·PHP·Rails·Spring 호환)를 경유할 것. 더 일반적으로, **소비자가 반환하는 구조의 자유도를 넓히는 API를 추가할 때는 그 값이 흘러가는 경로(직렬화·비교·저장)도 함께 넓혔는지 확인**한다 — 이 버그는 v2.5에서 `dataSource.request`로 반환 구조를 자유화하면서 직렬화는 평면 전제 그대로 둔 데서 생겼다.
