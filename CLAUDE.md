...
### 4. 예제 페이지 반영 규칙 (스펙 구현 직후 필수 판단)

**구현한 스펙이 예제 페이지에 들어갈 수 있으면 반드시 추가한다.** 판단 기준:

| 스펙 성격 | 반영할 곳 |
|---|---|
| 사용자가 눈으로 확인·조작할 수 있는 동작 (거의 모든 스펙) | `examples/features.html`에 Example/View Source 카드 **필수** — 전용 섹션(h2 + 설명 + 카드)으로 추가 |
...
| API 전용이라 데모가 무의미한 경우 (예: `getJson()` 같은 단순 getter) | 생략 가능하되, 가까운 기존 카드의 코드/주석에 사용 예를 한 줄이라도 노출. 완전히 생략하면 커밋 메시지에 사유 명시 |
