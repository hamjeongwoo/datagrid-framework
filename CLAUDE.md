...
| 파일 | 역할 |
|---|---|
| `dist/datagrid.js` | 프레임워크 본체 (단일 IIFE). 섹션: 헬퍼/아이콘 → **i18n(로케일 표 + `resolveLocaleText`)** → 순수 데이터 로직 → 컬럼 정규화 → **popupEditor 순수 함수** → Emitter → DataGrid 클래스(DOM 스캐폴딩 → 헤더 → 레이아웃 → 가상화 바디 → 정렬 → 필터 메뉴 → 트리 → 선택 → **에디터 위젯 팩토리(`_createEditorWidget`)** → 인라인 편집 → **팝업 폼 편집** → 클립보드 → 리사이즈/리오더 → 페이지네이션 → 오버레이 → 원격 dataSource → 데이터 API → 변경추적/undo → 상태 → CSV/Excel) → 내장 렌더러 → `_test` 노출 |
