...
### 4. 문서 갱신 체크리스트 (전부 필수 — 하나라도 빠지면 미완성)
- [ ] `docs/api-data.js` — 해당 섹션에 항목 추가, `since: '1.x.0'` 명시. 새 카테고리면 섹션 추가(구조는 파일 상단 주석 참조)
- [ ] `examples/features.html` — Example/View Source 카드 추가. **자체 완결형 코드**(복사하면 그대로 실행)로 작성. 마크업 규칙은 `demo/example-tabs.js` 상단 주석 참조
- [ ] `README.md` — 옵션/컬럼/API/이벤트 표에 항목 추가
- [ ] `docs/spec-roadmap.md` — 체크박스 `[x]` 처리, API명 바뀌었으면 반영
- [ ] 시각 컴포넌트가 새로 생겼으면 `examples/components.html` 갤러리에 견본 추가
