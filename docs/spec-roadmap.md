...
### v2.27.1 — 0건 결과가 그리드를 잠그던 버그 ([BUG-015](bug-reports/2026-09-18-015-empty-filter-result-locks-grid.md), 사용자 제보)
- 제보: "필터로 없는 데이터를 찾으면 그리드가 모두 disable 처리가 되어 아무것도 이용할 수 없다."
- `.dg-overlay`가 `inset: 0`으로 **루트 전체**(타이틀·툴바·헤더 포함)를 덮는데 `pointer-events` 선언이 없어 기본 `auto`였다. 결과가 0건이 되는 순간 헤더·필터 메뉴·플로팅 필터가 전부 클릭을 잃어 **필터를 되돌릴 UI 수단이 사라졌다.** `clearFilters()` 같은 프로그래매틱 경로는 멀쩡했다 — 데이터가 아니라 **입력이 막힌 것**.
- **"보여주는 것"과 "막는 것"은 다른 책임**이다. 오버레이는 표시 전용이므로 `pointer-events: none`(문구 패널만 `auto` — 드래그 복사), 진짜 잠금은 `setEnabled(false)`가 전담한다(§함정 9의 오버레이 판).
- 덮는 **범위**도 책임에 맞췄다 — `_positionOverlay()`가 `top = _bodyEl.offsetTop`으로 본문 영역에만 올린다. 루트를 덮으면 컬럼 이름까지 흐려져 **기능이 멀쩡해도 비활성처럼 읽힌다.** 높이는 CSS의 `bottom: 0`을 그대로 써 autoHeight처럼 본문이 짧은 레이아웃에서도 문구 자리가 남는다.
- 검증: 대조군/실험군이 수정 전엔 서로 다른 답(헤더 vs 오버레이), 수정 후엔 둘 다 헤더. **0건 상태에서 필터 메뉴를 열어 Clear로 20행 복구**(갇힘의 반대를 직접 증명). `setEnabled(false)`는 여전히 잠근다. normal/autoHeight/fill 세 레이아웃 + 라이트/다크, 콘솔 에러 0, 단위 테스트 1071 통과.
...
- **자식 그룹이 없는 구간은 상위 그룹이 두 줄을 차지한다.** 진짜 `rowspan`이 아니라 **칸 자체를 두 줄 높이(`height: calc(2 * var(--dg-group-header-height))`)로 키우고** 아래 줄의 몸통 칸(`dg-header-group-cont`)을 불투명 배경 + `z-index: 1`로 덮는 방식 — 본문 `mergeCells`의 병합 시작 셀이 이미 쓰는 패턴이다. 고정 컬럼은 아래 줄 고정 칸(`z-index: 3`)보다 위여야 하므로 `z-index: 4`(같은 3이면 DOM 순서로 아래 줄이 이긴다), `position`은 `sticky` 유지.
- **라벨 세로 정렬은 칸 크기로 푼다 — 라벨을 옮기지 않는다.** 처음엔 `translateY`로 라벨만 내렸는데 칸은 34px 그대로였고 `overflow: hidden`이라 **글자 아래가 잘렸다**(사용자 스샷 제보). 칸을 키우면 flex 정렬이 알아서 가운데로 놓고 `overflow`의 기준도 68px이 된다.
