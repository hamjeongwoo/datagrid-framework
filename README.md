...
| `editorSearch` | select·multiselect를 검색 패널로 — `true`(정적 목록 로컬 필터) 또는 `{ fetch(query) => Promise<options>, debounce, minLength, placeholder }` (lazy 검색).<br>`multiselect`에 주면(v2.25) 검색창 + 체크리스트가 되고 **고른 값이 칩으로 상시 표시**된다 — 필터로 목록에서 가려지거나 lazy 질의로 목록이 갈려도 선택이 유실되지 않는다. 칩의 `×`/항목 재클릭으로 해제. `Enter`는 **목록이 보일 때만** 활성 항목을 토글하고(v2.25.1), 목록이 접혀 있으면 폼 저장으로 넘어간다 — 마우스로 고른 직후의 `Enter`가 방금 고른 항목을 되돌리지 않는다. 이때 저장 순서는 `editorOptions` 순서가 아니라 **고른 순서**다 (lazy에는 전체 목록이 없다) |
