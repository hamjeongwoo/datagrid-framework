...
| `dataSource` | object | 원격 데이터 `{ url, method, params, request, parse, headers, paramsSerializer }` — `request(state)`로 요청 파라미터를 서버 스펙에 맞춤(중첩 객체·배열은 `page[selectPage]=1` 브래킷 표기로 직렬화), `parse(json)`으로 응답 변환, `headers`로 인증, `paramsSerializer(params)`로 쿼리스트링 생성 자체를 대체 (`reloadData()` 재요청, `setDataSource()` 교체) |
